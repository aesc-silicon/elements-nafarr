...
object AesMaskedAcceleratorCtrl {
  ...
  case class AesMaskedAcceleratorCtrl(p: Parameter) extends Component {
    ...
    val mCounter = Reg(UInt(log2Up(p.maskingDepth) bits))
    when(io.done || io.enable) {
      mCounter := 0
    }

    val ram = new Memory.RM_IHPSG13_1P_512x64_c2_bm_bist()
    ram.connectDefaults()
    ram.A_MEN := False
    ram.A_WEN := !enabled
    ram.A_REN := enabled
  ...
  case class Mapper(
    ...
    ctrl.enable := busCtrl.isWriting(0x0)
    ...
    val (stream, fifoOccupancy) = ctrl.ciphertext.queueWithOccupancy(p.ciphertextDepth)
    busCtrl.readStreamNonBlocking(
      stream,
      address = regOffset + 0xc
    )
