...
object I2cDeviceCtrl {
  ...
  case class IoFilter(i2c: I2c.Io, config: Config, p: Parameter) extends Area {
    ...
    val sampler = new Area {
      ...
      val sclSamples =
        History(that = sclSync, range = 0 until p.samplerWidth, when = clockDivider.io.tick, init = True)
      val sdaSamples =
        History(that = sdaSync, range = 0 until p.samplerWidth, when = clockDivider.io.tick, init = True)
    }
