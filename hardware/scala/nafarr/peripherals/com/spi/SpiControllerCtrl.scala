...
object SpiControllerCtrl {
  ...
  case class Mapper(
    ...
    val config = new Area {
      ...
      busCtrl.driveAndRead(cfg.cs.activeHigh, address = regOffset + 0x14)
      busCtrl.read(cfg.cs.setup, regOffset + 0x18)
      busCtrl.read(cfg.cs.hold, regOffset + 0x1c)
      busCtrl.read(cfg.cs.disable, regOffset + 0x20)
