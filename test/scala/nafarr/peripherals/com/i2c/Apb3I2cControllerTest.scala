...
class Apb3I2cControllerTest extends AnyFunSuite {
  ...
  test("parameters") {
    ...
    generationShouldFail(genCore(I2cControllerCtrl.Parameter(io = I2c.Parameter(0), timeoutWidth = 0)))
  }
  ...
  test("basic") {
    ...
    compiled.doSim("basicRegisters") { dut =>
      ...
      assert(
        apb.read(BigInt(staticOffset)) == BigInt("00001010", 16),
        "Unable to read 00001010 from I2cController clockDivier and timeout width declaration"
      )
